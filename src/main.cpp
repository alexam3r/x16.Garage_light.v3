...
void handleMotionSet(const char* cmd) {
    if (strcasecmp(cmd, PAYLOAD_ON) == 0) {
        ...
        motionEnabled = false;
        Serial.println(F("Motion reaction: OFF"));
    } else {
        Serial.print(F("motion/set: unknown payload: "));
        Serial.println(cmd);
    }
    // Флаг реакции намеренно НЕ публикуется — разовая команда без retain/state-эха.
}
...
// JSON-статус раз в минуту: light, mode, motion, temperature, humidity, uptime, wifi_rssi.
